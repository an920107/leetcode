  ...
  public:
    int recursive(vector<int> &nums, int target, int left, int right) {
        ...
        if (nums[left] <= target && target <= nums[mid]) {
            if (binary_search(nums.begin() + left, nums.begin() + mid + 1,
                              target)) {
                return lower_bound(nums.begin() + left, nums.begin() + mid + 1,
                                   target) -
                       nums.begin();
            }
            return -1;
        ...
        if (nums[mid] <= target && target <= nums[right]) {
            if (binary_search(nums.begin() + mid, nums.begin() + right + 1,
                              target)) {
                return lower_bound(nums.begin() + mid, nums.begin() + right + 1,
                                   target) -
                       nums.begin();
            }
            return -1;
